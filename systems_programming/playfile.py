...
class Winstart(MediaTool):                          # use Windows registry
    def open(self, medialfile, wait=False, **other):    # or os.system('start file')
        if not wait:
            os.startfile(mediafile)
        else:
            os.system('start /WAIT' + mediafile)
            
class Webbrowser(MedialTool):
    # file:// requires abs path
    def open(self, medialfile, **options):
        webbrowser.open_new('file://%s' % medialfile, **options)
...
videotools = {
    'linux2':   Cmdline('tkcVidio_c700 %s'),        # zaurus pda
...
app tools = {
    'win32':    Winstart()                          # doc, xls, etc: use at your owm risk
}
...
def playfile(filename, mimetable=mimetable, **options):
    ...
    if maintype == 'text' and subtype == 'html':
        ...
        playknownfile(filename, mimetable[maintype], **options) # key table
    else:
