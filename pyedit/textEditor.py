...
from gui_programming.guimaker import *         # Frame + menu/toolbar builders
...
class TextEditor:                       # mix with menu/toolbar Frame class
    ...
    # import in class to allow overrides in subclass or self
    if __name__ == '__main__':
        from textConfig import (        # my dir is on the path
            openAskUser, opensEncoding,
            savesUseKnownEncoding, saveAskUser, savesEncoding)
    else:
    ...
    def makeWidgets(self):                          # run by GuiMaker.__init__
        ...
        text.config(undo=1, autoseparators=1)       # 2.0, defaults is 0, 1
    ...
    def onOpen(self, loadFirst='', loadEncode=''):
        ...
        text = None                     # emtpy file = '' = False: test for None
        if loadEncode:
            try:
    ...
    def onSaveAs(self, forcefile=None):
        ...
        if self.knownEncoding and (                                 # enc known?
            (forcefile and self.savesUseKnowEncoding >= 1) or        # on Save?
        ...
        if not encpick and self.saveAskUser:
            self.update()           # else dialog doesn't appear in rare cases
    ...
    def onRedo(self):                           # 2.0: redo on undone
        try:
            self.text.edit_redo()
        except TclError:
    ...
    def onGrep(self):
        ...
        listbox clicks open matched file at line of occurence;
        ...
        we do for opens), but allow the the encoding used for the entire
        ...
        """
        from gui_programming.formrows import makeFormRow
    ...
    def grepThreadProducer(self, filenamepatt, dirname, grepkey, encoding, myqueue):
        ...
        """
        from systems_programming.find import find
        matches = []
        try:
    ...
    def onRunCode(self, parallelmode=True):
        ...
        from gui_programming.launchmodes import System, Start, StartArgs, Fork
        filemode = False
