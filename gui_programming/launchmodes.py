...
sub process module would work too, but os.popen() uses it internally, and the goal
...
new in the edition: run script filename path through normpath() to change any
/ or \ for windows tools where required; fix is inherited by PyEdit and others;
on windows, / is generally allowed for file opens, but not all launcher tools;
################################################################################
"""
...
def fixWindowsPath(cmdline):
    """
    change all / to \ in script filename path at fron of cmdline;
...
class Popen(LaunchMode):
    ...
    caveat: may block caller, since pipe closed to soon
    """
    def run(self, cmdline):
        cmdline = fixWindowsPath(cmdline)
...
def selftest():
    ...
    if sys.platform[:3] == 'win':
        input('DOS start mode...')              # no blocks
        StartArgs(file, file)()
