...
actions to be run un the main thread; this doesn't block the GUI -it just
...
# run ever if no threads                    # in standard lib now
try:                                        # raise import error to
...
################################################################################
# IN MAIN THREAD - periodically cheach thread completions queue; run implied GUI
...
def threadChecker(widget, delayMsecs=100, perEvent=1):       # 10x/sec, 1/timer
    ...
    widget.after(delayMsecs,                                # reset timer event 
        lambda: threadChecker(widget, delayMsecs, perEvent))# back ti event loop
...
################################################################################
# self-test code: splite thread action into main, exits, progress
################################################################################

if __name__ == '__main__':                          # self-test code when run
    import time                                     # or scrolledtext
